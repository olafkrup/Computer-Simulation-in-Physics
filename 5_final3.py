...
kB = 1
m = 1
v_mean = np.sqrt(kB * T / m) #mean velocity in one direction
...
class Particle:
    ...
    def Update_position(self, dt): # Leapfrog algorithm
        if self.v_half is None:
            self.v_half = self.v - 0.5* self.F * dt
        v = self.v_half
        self.v_half = self.v_half + self.F * dt
...
def initialise(N, v_mean, sigma, size, box_size):
...
initialise(N, v_mean, variance, size, box_size)
...
def image_update(i):
    ...
    # k = np.append(Parameters, [[E_k, E_p, E, T, P]])
    textstr = '\n'.join((
        r'$E_k=%.2f$' % (E_k,),
        r'$E_p=%.2f$' % (E_p,),
        r'$E=%.2f$' % (E,),
